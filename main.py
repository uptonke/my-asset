...
# 🔧 ROOT FIX：清除 yfinance SQLite cache，防止 CI 環境鎖死
...
bonds_raw = yf.download([”^TNX”, “^IRX”], period=“1mo”, progress=False, auto_adjust=True)

```
# 防護：相容 MultiIndex 與單層 columns
if isinstance(bonds_raw.columns, pd.MultiIndex):
    bonds = bonds_raw["Close"]
elif "Close" in bonds_raw.columns:
    bonds = bonds_raw["Close"]
else:
    bonds = bonds_raw

y10_series = bonds["^TNX"] if "^TNX" in bonds.columns else pd.Series(dtype=float)
y3m_series = bonds["^IRX"] if "^IRX" in bonds.columns else pd.Series(dtype=float)

y10_clean = y10_series.dropna()
y10 = float(y10_clean.iloc[-1]) if not y10_clean.empty else 4.5   # fallback

y3m_clean = y3m_series.dropna()
y3m = float(y3m_clean.iloc[-1]) if not y3m_clean.empty else 5.0   # fallback
...
# E. 抓取機構級情緒、通膨與實體經濟指標 (VIX, DXY, Gold, Copper, Oil)
...
try:
    ...
    copper = float(yf.Ticker("HG=F").history(period="5d")['Close'].iloc[-1])
...
```
    ...
    if all_tickers:
        ...
        # 建立 original ticker → yf ticker 對照表
        ...
        if valid_investable:
            ...
            if num_assets < 5:
                ...
                dynamic_min = min(0.03, 0.9 / num_assets)
                dynamic_max = 0.20

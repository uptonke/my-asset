(() => {
  "use strict";
  const PANEL_ID = "quant-meta-inline-panel-v1";
  const STYLE_ID = "quant-meta-inline-style-v1";
  ...
  function injectStyle() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      #${PANEL_ID}{margin-bottom:16px;border:1px solid rgba(255,255,255,.12);border-radius:18px;background:rgba(15,23,42,.72);backdrop-filter:blur(18px);box-shadow:0 18px 45px rgba(0,0,0,.24);overflow:hidden}
      ...
      #${PANEL_ID} .qmp-sub{color:rgba(203,213,225,.76);font-size:11px;line-height:1.6;margin-top:5px}
      ...
      #${PANEL_ID} th{position:sticky;top:0;z-index:1;background:rgba(15,23,42,.94);color:#94a3b8;font-size:10px;text-transform:uppercase;letter-spacing:.08em;font-weight:800;padding:10px;border-bottom:1px solid rgba(255,255,255,.10);text-align:right;white-space:nowrap}
      ...
      #${PANEL_ID} th:first-child,#${PANEL_ID} td:first-child{text-align:left;position:sticky;left:0;background:rgba(15,23,42,.96);z-index:2}
      ...
      #${PANEL_ID} .src{max-width:180px;overflow:hidden;text-overflow:ellipsis;display:inline-block;vertical-align:bottom;color:#93c5fd}
  ...
  function getRows(proxy) {
    const raw = proxy?.filteredHoldingsGroups || proxy?.groupedHoldings || {};
    const rows = [];
    if (!raw || typeof raw !== "object") return rows;
    Object.entries(raw).forEach(([category, group]) => {
      (Array.isArray(group?.items) ? group.items : []).forEach((stock) => rows.push({ category, stock }));
    });
    return rows;
  }
  ...
  function sourceLabel(value) {
    return String(value || "N/A").replace("YahooChart:", "YC:").replace("yfinance:", "YF:");
  }
  ...
  function buildRow(proxy, entry) {
    const stock = entry.stock || {};
    const ticker = String(stock.ticker || "").toUpperCase();
    const meta = proxy?.stockMeta?.[ticker] || {};
    const beta = meta.beta ?? stock.beta;
    return `
      <tr>
        <td><span class="ticker">${escapeHtml(ticker)}</span></td>
        <td><span class="badge">${num(beta, 2)}</span></td>
  ...
  function render() {
    ...
    const rows = getRows(proxy);
    const stockMeta = proxy.stockMeta || {};
    const latestUpdated = rows
      .map(({ stock }) => stockMeta?.[String(stock?.ticker || "").toUpperCase()]?.updated_at)
      .filter(Boolean)
      .sort()
      .slice(-1)[0] || "N/A";
    ...
    panel.innerHTML = `
      <div class="qmp-head">
        <div>
          ...
          <div class="qmp-sub">來自 Supabase stock_meta。Beta 顯示小數點後兩位；Risk 越高代表風險越高，其餘 score 越高通常越佳。</div>
        </div>
        <div class="qmp-meta">holdings=${rows.length}<br>latest=${escapeHtml(latestUpdated)}</div>
      </div>
      <div class="qmp-wrap">
        <table>
          <thead>
          ...
          <tbody>${rows.map((entry) => buildRow(proxy, entry)).join("")}</tbody>
        </table>
      </div>`;
  }
  ...
  function boot() {
    ...
    const timer = setInterval(() => {
      tries += 1;
      render();
      if (tries >= 10 && getVueProxy()) clearInterval(timer);
    }, 800);
    document.addEventListener("click", () => setTimeout(render, 200), true);
    document.addEventListener("change", () => setTimeout(render, 200), true);

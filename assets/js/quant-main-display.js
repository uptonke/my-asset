(() => {
  ...
  function renderQuant(data) {
    ...
    const rows = list.map((t) => {
      ...
      return `<tr><td><span class="ticker">${esc(t)}</span></td><td>${n(x.beta,2)}</td><td><span class="badge ${scoreClass(x.trend_score)}">${n(x.trend_score,2)}</span></td><td><span class="badge ${scoreClass(x.momentum_score)}">${n(x.momentum_score,2)}</span></td><td><span class="badge ${scoreClass(x.risk_score,true)}">${n(x.risk_score,2)}</span></td><td>${n(x.technical_score,2)}</td><td>${n(x.valuation_score,2)}</td><td><span class="badge ${scoreClass(x.quant_health_score)}">${n(x.quant_health_score,2)}</span></td><td>${zone(x,"support")}</td><td>${zone(x,"resistance")}</td><td>${esc(position)}</td><td><span class="badge ${signalClass(signal)}">${esc(signal)}</span>${reason ? `<span class="qmi-small">${esc(reason)}</span>` : ""}</td><td>${esc(x.data_quality || "N/A")}</td><td>${esc(x.source || "N/A")}</td><td>${esc(x.updated_at || "N/A")}</td></tr>`;
    }).join("");
    p.innerHTML = `<div class="qmi-head"><div><div class="qmi-title">量化因子總覽</div><div class="qmi-sub">庫存頁只放單檔資料：Beta、趨勢、動能、風險、健康度與月線支撐/壓力；籌碼保留為台股附加資料，不納入跨資產核心 Factor。</div></div><div class="qmi-meta">持倉數=${list.length}<br>最新=${esc(latest)}</div></div><div class="qwrap"><table><thead><tr><th>代號</th><th>Beta</th><th>趨勢</th><th>動能</th><th>風險</th><th>技術</th><th>估值</th><th>健康度</th><th>月支撐</th><th>月壓力</th><th>月線位置</th><th>綜合訊號</th><th>品質</th><th>來源</th><th>更新</th></tr></thead><tbody>${rows}</tbody></table></div>`;
  }

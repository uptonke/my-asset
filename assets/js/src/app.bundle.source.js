...
const safeJumpTailLoss = Math.abs(bestPort.jumpTailLoss) < 1e-8
    ...
    chartMC = new Chart(ctx, {
        ...
        const ZH_TEXT_MAP = Object.freeze({
            ...
            'missing_human_confirmed_ids': '缺少人工確認票據 ID',
            'no_manual_trade_ticket_rows': '沒有人工交易票據草案',
            'manual_trade_ticket_not_available': '人工交易票據草案不可用',
            'trade_sizing_diagnostics_blocked': '交易 sizing 診斷仍阻擋',
            'manual_override_disabled': '人工 override 未啟用',
            'cash_balance_missing': '現金餘額缺失',
            'not_all_prices_real_world': '並非全部價格為 real-world fetch',
            'confirmed_ids_not_found': '確認 ID 找不到',
...
chartCML.data.datasets = [
    ...
    return Array.isArray(rows) ? rows : [];
       ...
       const manualApprovalConsole = ref(null);
       const manualApprovalConsoleError = ref('');
       const manualApprovalConsoleLoading = ref(false);
       const manualApprovalConsoleLastLoadedAt = ref('');
       const manualApprovalConsoleReloadMessage = ref('');
       async function loadManualApprovalConsole() {
        manualApprovalConsoleError.value = '';
        manualApprovalConsoleLoading.value = true;
        manualApprovalConsoleReloadMessage.value = '正在重新載入審閱控制台資料...';
        const startedAt = Date.now();
        try {
            const response = await fetch('data/alpha/manual_approval_console_latest.json?_=' + startedAt, { cache: 'no-store' });
            if (!response.ok) {
                manualApprovalConsole.value = null;
                manualApprovalConsoleError.value = `尚未讀到 v9.4 人工審閱控制台 (${response.status})`;
                manualApprovalConsoleReloadMessage.value = `審閱控制台載入失敗：HTTP ${response.status}`;
                return;
            }
            const data = await response.json();
            manualApprovalConsole.value = data && typeof data === 'object' ? data : null;
            manualApprovalConsoleLastLoadedAt.value = reloadNowText();
            manualApprovalConsoleReloadMessage.value = `審閱控制台已重新載入：${manualApprovalConsoleLastLoadedAt.value}`;
        } catch (err) {
            manualApprovalConsole.value = null;
            manualApprovalConsoleError.value = err?.message || String(err);
            manualApprovalConsoleReloadMessage.value = `審閱控制台載入失敗：${manualApprovalConsoleError.value}`;
        } finally {
            manualApprovalConsoleLoading.value = false;
        }
       }
       const manualApprovalConsoleSummary = computed(() => manualApprovalConsole.value?.summary || {});
       const manualApprovalMissingInputs = computed(() => Array.isArray(manualApprovalConsole.value?.missing_inputs) ? manualApprovalConsole.value.missing_inputs : []);

       const paperTradePerformanceEvaluator = ref(null);
       const paperTradePerformanceError = ref('');
       const paperTradePerformanceLoading = ref(false);
       const paperTradePerformanceLastLoadedAt = ref('');
       const paperTradePerformanceReloadMessage = ref('');
       async function loadPaperTradePerformanceEvaluator() {
        paperTradePerformanceError.value = '';
        paperTradePerformanceLoading.value = true;
        paperTradePerformanceReloadMessage.value = '正在重新載入紙上績效資料...';
        const startedAt = Date.now();
        try {
            const response = await fetch('data/alpha/paper_trade_performance_evaluator_latest.json?_=' + startedAt, { cache: 'no-store' });
            if (!response.ok) {
                paperTradePerformanceEvaluator.value = null;
                paperTradePerformanceError.value = `尚未讀到 v9.5 紙上績效評估 (${response.status})`;
                paperTradePerformanceReloadMessage.value = `紙上績效載入失敗：HTTP ${response.status}`;
                return;
            }
            const data = await response.json();
            paperTradePerformanceEvaluator.value = data && typeof data === 'object' ? data : null;
            paperTradePerformanceLastLoadedAt.value = reloadNowText();
            paperTradePerformanceReloadMessage.value = `紙上績效已重新載入：${paperTradePerformanceLastLoadedAt.value}`;
        } catch (err) {
            paperTradePerformanceEvaluator.value = null;
            paperTradePerformanceError.value = err?.message || String(err);
            paperTradePerformanceReloadMessage.value = `紙上績效載入失敗：${paperTradePerformanceError.value}`;
        } finally {
            paperTradePerformanceLoading.value = false;
        }
       }
       const paperTradePerformanceSummary = computed(() => paperTradePerformanceEvaluator.value?.summary || {});
       const paperTradePerformanceRows = computed(() => Array.isArray(paperTradePerformanceEvaluator.value?.evaluation_rows) ? paperTradePerformanceEvaluator.value.evaluation_rows : []);
       const paperTradePerformanceVisibleRows = computed(() => paperTradePerformanceRows.value.slice(0, 5));

       const tradeTicketExplainability = ref(null);
       const tradeTicketExplainabilityError = ref('');
       const tradeTicketExplainabilityLoading = ref(false);
       const tradeTicketExplainabilityLastLoadedAt = ref('');
       const tradeTicketExplainabilityReloadMessage = ref('');
       async function loadTradeTicketExplainability() {
        tradeTicketExplainabilityError.value = '';
        tradeTicketExplainabilityLoading.value = true;
        tradeTicketExplainabilityReloadMessage.value = '正在重新載入票據解釋資料...';
        const startedAt = Date.now();
        try {
            const response = await fetch('data/alpha/trade_ticket_explainability_latest.json?_=' + startedAt, { cache: 'no-store' });
            if (!response.ok) {
                tradeTicketExplainability.value = null;
                tradeTicketExplainabilityError.value = `尚未讀到 v9.6 票據解釋 (${response.status})`;
                tradeTicketExplainabilityReloadMessage.value = `票據解釋載入失敗：HTTP ${response.status}`;
                return;
            }
            const data = await response.json();
            tradeTicketExplainability.value = data && typeof data === 'object' ? data : null;
            tradeTicketExplainabilityLastLoadedAt.value = reloadNowText();
            tradeTicketExplainabilityReloadMessage.value = `票據解釋已重新載入：${tradeTicketExplainabilityLastLoadedAt.value}`;
        } catch (err) {
            tradeTicketExplainability.value = null;
            tradeTicketExplainabilityError.value = err?.message || String(err);
            tradeTicketExplainabilityReloadMessage.value = `票據解釋載入失敗：${tradeTicketExplainabilityError.value}`;
        } finally {
            tradeTicketExplainabilityLoading.value = false;
        }
       }
       const tradeTicketExplainabilitySummary = computed(() => tradeTicketExplainability.value?.summary || {});
       const tradeTicketExplainabilityRows = computed(() => Array.isArray(tradeTicketExplainability.value?.explanation_rows) ? tradeTicketExplainability.value.explanation_rows : []);
       const tradeTicketExplainabilityVisibleRows = computed(() => tradeTicketExplainabilityRows.value.slice(0, 5));


       const nativeTargetThesis = ref(null);
       const nativeTargetThesisError = ref('');
       async function loadNativeTargetThesis() {
        nativeTargetThesisError.value = '';
        try {
            ...
            const data = await response.json();
            nativeTargetThesis.value = data && typeof data === 'object' ? data : null;
        } catch (err) {
            nativeTargetThesis.value = null;
            nativeTargetThesisError.value = err?.message || String(err);
        }
       }
       const nativeTargetThesisSummary = computed(() => nativeTargetThesis.value?.summary || {});
       const nativeTargetThesisRows = computed(() => Array.isArray(nativeTargetThesis.value?.thesis_rows) ? nativeTargetThesis.value.thesis_rows : []);
       const nativeTargetThesisVisibleRows = computed(() => nativeTargetThesisRows.value.slice(0, 8));
       const nativeTargetThesisAgreementCounts = computed(() => nativeTargetThesisSummary.value?.source_agreement_counts || {});



       const delegatedTargetWeightDraft = ref(null);
       const delegatedTargetWeightDraftError = ref('');
       async function loadDelegatedTargetWeightDraft() {
        delegatedTargetWeightDraftError.value = '';
        try {
            ...
            const data = await response.json();
            delegatedTargetWeightDraft.value = data && typeof data === 'object' ? data : null;
        } catch (err) {
            delegatedTargetWeightDraft.value = null;
            delegatedTargetWeightDraftError.value = err?.message || String(err);
        }
       }
       const delegatedTargetWeightDraftSummary = computed(() => delegatedTargetWeightDraft.value?.summary || {});
       const delegatedTargetRows = computed(() => Array.isArray(delegatedTargetWeightDraft.value?.machine_target_rows) ? delegatedTargetWeightDraft.value.machine_target_rows : []);
       const delegatedTargetVisibleRows = computed(() => delegatedTargetRows.value.slice(0, 12));
       const delegatedDraftLines = computed(() => Array.isArray(delegatedTargetWeightDraft.value?.delegated_draft_lines) ? delegatedTargetWeightDraft.value.delegated_draft_lines : []);
       const delegatedDraftVisibleLines = computed(() => delegatedDraftLines.value.slice(0, 10));
       const delegatedDraftWarnings = computed(() => Array.isArray(delegatedTargetWeightDraft.value?.warnings) ? delegatedTargetWeightDraft.value.warnings : []);
       const delegatedDraftVisibleWarnings = computed(() => delegatedDraftWarnings.value.slice(0, 6));

       const dailyQuantReference = ref(null);
       const dailyQuantReferenceError = ref('');
       async function loadDailyQuantReference() {
        dailyQuantReferenceError.value = '';
        try {
            const response = await fetch('data/alpha/daily_quant_reference_latest.json?_=' + Date.now(), { cache: 'no-store' });
            if (!response.ok) {
                dailyQuantReference.value = null;
                dailyQuantReferenceError.value = `尚未讀到 v10.5 Daily Quant 參考層 (${response.status})`;
                return;
            }
            const data = await response.json();
            dailyQuantReference.value = data && typeof data === 'object' ? data : null;
        } catch (err) {
            dailyQuantReference.value = null;
            dailyQuantReferenceError.value = err?.message || String(err);
        }
       }
       const dailyQuantReferenceSummary = computed(() => ({
        portfolio_source: dailyQuantReference.value?.portfolio_source,
        daily_update_workflow_exists: dailyQuantReference.value?.daily_update_workflow?.exists,
        synthetic_risk_available: dailyQuantReference.value?.synthetic_risk_reference?.available,
        monte_carlo_reference_available: dailyQuantReference.value?.monte_carlo_reference?.available,
        stock_meta_count: dailyQuantReference.value?.stock_meta_reference?.stock_meta_count,
        price_map_count: dailyQuantReference.value?.settings_reference?.price_map_count,
        liquidity_buffer_ratio: dailyQuantReference.value?.settings_reference?.liquidity_buffer_ratio,
        exchange_rate: dailyQuantReference.value?.settings_reference?.exchange_rate
       }));

       const delegatedDraftMonteCarloCheck = ref(null);
       const delegatedDraftMonteCarloCheckError = ref('');
       async function loadDelegatedDraftMonteCarloCheck() {
        delegatedDraftMonteCarloCheckError.value = '';
        try {
            const response = await fetch('data/alpha/delegated_draft_monte_carlo_check_latest.json?_=' + Date.now(), { cache: 'no-store' });
            if (!response.ok) {
                delegatedDraftMonteCarloCheck.value = null;
                delegatedDraftMonteCarloCheckError.value = `尚未讀到 v10.5 Monte Carlo 參考檢查 (${response.status})`;
                return;
            }
            const data = await response.json();
            delegatedDraftMonteCarloCheck.value = data && typeof data === 'object' ? data : null;
        } catch (err) {
            delegatedDraftMonteCarloCheck.value = null;
            delegatedDraftMonteCarloCheckError.value = err?.message || String(err);
        }
       }
       const delegatedDraftMonteCarloSummary = computed(() => delegatedDraftMonteCarloCheck.value?.summary || {});
       const delegatedDraftMonteCarloChecks = computed(() => Array.isArray(delegatedDraftMonteCarloCheck.value?.reference_checks) ? delegatedDraftMonteCarloCheck.value.reference_checks : []);
       const delegatedDraftMonteCarloVisibleChecks = computed(() => delegatedDraftMonteCarloChecks.value.slice(0, 6));
       const delegatedDraftMonteCarloRiskNotes = computed(() => Array.isArray(delegatedDraftMonteCarloCheck.value?.risk_notes) ? delegatedDraftMonteCarloCheck.value.risk_notes : []);
       const delegatedDraftMonteCarloVisibleRiskNotes = computed(() => delegatedDraftMonteCarloRiskNotes.value.slice(0, 5));

       const humanConfirmedTradeTicket = ref(null);
       const humanConfirmedTradeTicketError = ref('');
       async function loadHumanConfirmedTradeTicket() {
        humanConfirmedTradeTicketError.value = '';
        try {
            const response = await fetch('data/alpha/human_confirmed_trade_ticket_latest.json?_=' + Date.now(), { cache: 'no-store' });
            if (!response.ok) {
                humanConfirmedTradeTicket.value = null;
                humanConfirmedTradeTicketError.value = `尚未讀到 v10.0 人工確認交易票據 (${response.status})`;
                return;
            }
            const data = await response.json();
            humanConfirmedTradeTicket.value = data && typeof data === 'object' ? data : null;
        } catch (err) {
            humanConfirmedTradeTicket.value = null;
            humanConfirmedTradeTicketError.value = err?.message || String(err);
        }
       }
       const humanConfirmedTradeTicketSummary = computed(() => humanConfirmedTradeTicket.value?.summary || {});
       const humanConfirmedTicketRows = computed(() => Array.isArray(humanConfirmedTradeTicket.value?.confirmed_ticket_rows) ? humanConfirmedTradeTicket.value.confirmed_ticket_rows : []);
       const humanConfirmedTicketVisibleRows = computed(() => humanConfirmedTicketRows.value.slice(0, 5));
       const humanConfirmedTicketBlockers = computed(() => Array.isArray(humanConfirmedTradeTicket.value?.blockers) ? humanConfirmedTradeTicket.value.blockers : []);
       const humanConfirmedTicketVisibleBlockers = computed(() => humanConfirmedTicketBlockers.value.slice(0, 6));
    ...
    return Array.isArray(rows) ? rows : [];
       ...
       const optimizerDataReloading = ref(false);
       const optimizerDataLastLoadedAt = ref('');
       const optimizerDataReloadMessage = ref('');

       async function reloadAllOptimizerData(options = {}) {
        const isInitial = options?.initial === true;
        optimizerDataReloading.value = true;
        optimizerDataReloadMessage.value = isInitial ? '正在載入全部最佳化資料...' : '正在重新載入全部最佳化資料...';
        const tasks = [
            ...
            loadManualApprovalConsole,
            loadPaperTradePerformanceEvaluator,
            loadTradeTicketExplainability,
            loadNativeTargetThesis,
            loadDelegatedTargetWeightDraft,
            loadDailyQuantReference,
            loadDelegatedDraftMonteCarloCheck,
            loadHumanConfirmedTradeTicket
        ];
        try {
            await Promise.allSettled(tasks.map((fn) => fn()));
            optimizerDataLastLoadedAt.value = reloadNowText();
            optimizerDataReloadMessage.value = isInitial
                ? `全部最佳化資料已載入：${optimizerDataLastLoadedAt.value}`
                : `全部最佳化資料已重新載入：${optimizerDataLastLoadedAt.value}`;
        } catch (err) {
            optimizerDataReloadMessage.value = `全部資料重新載入失敗：${err?.message || String(err)}`;
        } finally {
            optimizerDataReloading.value = false;
    ...
    loadDataFromCloud();
    reloadAllOptimizerData({ initial: true });

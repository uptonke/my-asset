...
def _fit_gpd_at_tail_fraction(
    ...
    *,
    ...
    if tail_prob <= 0 or fu <= tail_prob:
        # The POT formula here is intended for a target tail probability deeper
        # than the threshold exceedance rate.
        return None
...
def compute_evt_robustness(
    ...
    *,
    ...
    The primary fit uses the top ``primary_tail_fraction`` of daily losses.
    Threshold sensitivity is reported across ``threshold_grid``. A circular
    moving-block bootstrap resamples the full daily return sequence, recomputes
    the primary threshold in each sample, and refits the GPD. The resulting
    percentile intervals measure estimation uncertainty, not future-return
    predictive ranges.
    """
    base = _empty_payload(
        alpha=alpha,
    ...
    try:
        ...
        base.update({
            ...
            "finite_upper_endpoint_loss_pct": round(float(primary["finite_upper_endpoint_loss_pct"]), 4)
            if primary.get("finite_upper_endpoint_loss_pct") is not None else None,

...
- **blocker** `incomplete_real_world_price_fetch` — 部分資產價格不是即時真實抓取：未成功 real-world fetch 的標的：00981A, AVUV, BOXX, BTC-USD, ETH-USD, GLDM, IFRA, PICK, QQQ, SRVR, USMV, VEA。
- **watch** `minimum_trade_unit_not_met` — 部分部位低於最小交易單位：共有 1 檔不符合最小單位檢查。
